...
case_enct = read.table("..\\case\\Encounter_Patient.csv",sep = ",", fill = TRUE ,
...
case_cevnt = read.table("..\\case\\Clinical_Events.csv",sep = ",", fill = TRUE ,
...
case_diag =  read.table("..\\case\\Diagnosis.csv" ,sep = ",", fill = TRUE ,
                        header = TRUE)[,c("ï..ENCOUNTER_ID","DIAGNOSIS_DESCRIPTION","DIAGNOSIS_TYPE_DISPLAY" )]


case_lab =   read.table("..\\case\\Lab.csv"  ,sep = ",", fill = TRUE ,
...
#case_med =   read.table("..\\case\\Medical.csv"  ,sep = ",", fill = TRUE , header = TRUE)
#case_proc =  read.table("..\\case\\Procedural.csv" ,sep = ",", fill = TRUE , header = TRUE)

list_groups_cevnt= c("Blood Pressure","BMI BSA","Alcohol","Smoke/Tobacco","Heart Rate")
case_cevnt = case_cevnt[case_cevnt$`EVENT_CODE_GROUP` %in% list_groups_cevnt,]
#unique(case_cevnt$EVENT_CODE_GROUP)
list_groups_lab= c("Cholesterol Test","Glucose Test")
case_lab = case_lab[case_lab$LAB_PROCEDURE_GROUP %in% list_groups_lab,]



#x = ddply(case_enct,.(PATIENT_SK),nrow)  # encounter per each patient
x =NULL
enct_lab = data.frame()
...
case_enct$AGE_IN_YEARS = as.numeric(as.character(case_enct$AGE_IN_YEARS))
i=NULL
...
for (i in sk){
  ...
  print(i)
  encounters_patient = case_enct[case_enct$PATIENT_SK == i,]
  
  enct_cenvt = join(encounters_patient, case_cevnt, by = c("ï..ENCOUNTER_ID" = "ï..ENCOUNTER_ID"),type= "inner")
  write.csv(enct_cenvt, file = "..\\control\\enct_cenvt_merged.csv", row.names = FALSE)
  enct_lab = join(encounters_patient, case_lab, by = c("ï..ENCOUNTER_ID" = "ï..ENCOUNTER_ID"),type= "inner")
